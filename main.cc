...
{
  public:
    enum Status { SLEEP = 0, RUNNING, STOPPED };

    template<typename F, typename... Args>
    explicit Context(F&& f, Args&&... args)
      : state_{SLEEP}
    {
      bp = malloc(STACK_SIZE);
      sp = init_stack((char*)bp - RED_ZONE, (void*)ctx_function<Context>);
      printf("original stack: %p, sp: %p\n", bp, sp);
      fn_ = [fn = std::forward<F>(f), params = std::make_tuple(std::forward<Args>(args)...)] {
        std::apply(fn, params);
      };
    }
  ...
  private:
    enum { RED_ZONE = 128, STACK_SIZE = 2048 };
    Status state_;
    void* bp; // actually it's including RED_ZONE
    void* sp;
    std::function<void()> fn_;

    void set_status(Context::Status state)
    {
      state_ = state;
    }
};
...
class Scheduler final
{
  ...
  public:
    ...
    {
      queue_.emplace_back(new Context{std::forward<F>(f), std::forward<Args>(args)...});
    }
  ...
  private:
    ...
    static void reset_timer(Scheduler* r)
    {
      timer_settime(r->timer(), 0, fixed_timeout(), NULL);
    }
    ...
    void context_switch()
    {
      if(queue_.empty())
      {
        ...
        Context::context_switch(&last, &main_ctx_);
      }
      ...
      // if there's only one left, there are two branches
      if(next == prev)
      {
        if(next->running())
        {
        ...
        if(next->sleep())
        {
          // since prev sp already de-allocated, use tmp instead
          Context tmp;
          Context::context_switch(&tmp, next);
        }
      }
    ...
    void init_signal()
    {
      ...
      //timer_create(CLOCK_PROCESS_CPUTIME_ID, &sigev, &timer_);
...
void yield()
{
  Scheduler::instance()->context_switch();
}
...
int main()
{
  ...
  sh->spawn([] {
    for(int i = 0; i< 3; ++i) {
      spin();
      printf("\e[33mfoo: %d\e[0m\n", i);
    }
  });
  sh->spawn([] {
    for(int i = 0; i < 3; ++i) {
      spin();
      printf("\e[32mbar: %d\e[0m\n", i);
    }
  });
  sh->start();
}
